...
n_inn = nkerns[-1] * last_conv_fm_shape[0] * last_conv_fm_shape[1]
#import theano.printing as printing
#n_in = printing.Print('text')(n_in)
classification_branch = feature_representation_layers + [models2.LogisticRegression(layerIdx=len(nkerns)+1,n_in=n_inn, n_out=2)]


x_s_input = x_s.reshape((batch_size, image_size[0], image_size[1],nb_channels))
x_s_input = x_s_input.dimshuffle(0,3,1,2)

next_layer_input1 = x_s_input
next_layer_input2 = classification_branch[0].fprop(next_layer_input1)
next_layer_input3 = classification_branch[-1].fprop(next_layer_input2.flatten(2))

#p_y_given_x = next_layer_input.copy()

Lf_s = classification_branch[-1].negative_log_likelihood(next_layer_input3,yf_s)
...
ss = lambda_p / lambda_p 
ccost = ss * Lf_s #- lambda_p *( Ld_s + Ld_t)
