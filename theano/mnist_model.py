...
class DeepVamp(object):
    def __init__(self, dataset, 
    	...
    	self.layers = [models.LeNetConvPoolLayer(rng=self.rng,layerIdx=0, input=layer0_input, 
        ...
        output_h = self.layers[-1].out.flatten(2)
        self.layers += [models.HiddenLayer(
                             rng = self.rng,
                             layerIdx=len(hidden_size)+1,
                             input=output_h,
                             n_in=2304,
                             n_out=500,
                             activation=T.tanh
                             )]
        self.layers += [models.LogisticRegression(input=self.layers[-1].output,layerIdx=len(hidden_size)+2, n_in=500, n_out=2)]
        #outputlayer
        #cost
        cost = self.layers[-1].negative_log_likelihood(target)
    	self.cost = cost
        #parameter update
