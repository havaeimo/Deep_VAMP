...
def makepatches(image):
    ...
    patches = np.zeros(((height-64)*(width-64),64,64,3),dtype=np.float32)
    for i in range(height-64):
        for j in range(width-64):
            patches[idx,...] = image[i:i+64,j:j+64,:]
            idx += 1
    assert patches.shape[0] == (height-64)*(width-64)        
    return patches
...
def generate_prediction_patchwise(data,fprop,batch_size=100):
    ...
    for image in data:
        ...
        result_image = result_patches.reshape(height-64,width-64,2)
...
if __name__ == "__main__":
    ...
    result_path = '/home/local/USHERBROOKE/havm2701/data/deepvamp_results/cudaconvnet'
    ...
    name_testdata = name_testdata[:10]
