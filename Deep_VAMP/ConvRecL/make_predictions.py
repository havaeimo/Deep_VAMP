...
def resize_ground_truth(im_path,txt_path,im_size=(240,320)):
    ...
    for name in list_dir:
        img = Image.open(join(im_path,name.replace('.txt','.png')))
        #orig_shape = (img.size[1],img.size[0])
        orig_shape = img.size
        img = img.resize((320,240),PIL.Image.ANTIALIAS)
        scale =  (320.0/orig_shape[0],240.0/orig_shape[1])
        ...
        for line in content:
            line = line.replace(' \n','')
            l_c = [int(e) for e in line.split(' ')]
            x,y,w,h = l_c
            x_n = x * scale[0]
            w_n = w * scale[0]
            y_n = y * scale[1]
            h_n = h * scale[1]
            line= str(int(x_n)) + ' ' + str(int(y_n)) + ' ' + str(int(w_n)) + ' ' + str(int(h_n)) + ' ' + '\n' 
            g.write(line)
