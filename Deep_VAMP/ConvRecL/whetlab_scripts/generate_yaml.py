...
def fix_yaml_params(yaml_param):
    ...
    yaml_param_new['kernel_shape_G1'] = kernel_shape_G1_range[
        yaml_param['kernel_shape_G1']]

    kernel_shape_G2_range = [3, 5, 7]
    yaml_param_new['kernel_shape_G2'] = kernel_shape_G2_range[
        yaml_param['kernel_shape_G2']]

    pool_shape_G1_range = [2, 4]
    yaml_param_new['pool_shape_G1'] = kernel_shape_G1_range[
        yaml_param['pool_shape_G1']]

    pool_shape_G2_range = [2, 4]
    yaml_param_new['pool_shape_G2'] = kernel_shape_G2_range[
        yaml_param['pool_shape_G2']]
    ...
    yaml_param_new['decay_factor'] = decay_factor_rage[
        yaml_param['decay_factor']]
...
def fit_model(param, gpu):
    ...
    data = os.environ.get('HOME') + '/data/Data'
    ...
    train = os.environ.get('HOME') + '/libs/pylearn2/pylearn2/scripts/train.py'
    # import pdb
    # pdb.set_trace()
    ...
    with open(generated_yaml_path, 'w') as File_firstphase1:
    ...
    print "Done in {0} sec.".format(t.time() - start)

    gpu_flag = 'THEANO_FLAGS=device=' + gpu + ',floatX=float32'

    command = gpu_flag + ' ' + 'python' + ' ' + train + \
        ' ' + generated_yaml_path + '>' + logfile_path
    print command
    os.system(command)
